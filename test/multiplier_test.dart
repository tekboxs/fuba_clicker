...
void main() {
  group('Multiplicadores Totais com Riverpod', () {
    ...
    test('Multiplicador deve mudar ao equipar um acessório', () {
      ...
      expect(
          multiplierAfter.toDouble(), greaterThan(multiplierBefore.toDouble()));
    ...
    test('Multiplicador deve combinar rebirths e acessórios corretamente', () {
      ...
      final multiplierRebirthOnly =
          container.read(totalMultiplierProvider).toDouble();

      container.read(equippedAccessoriesProvider.notifier).state = [
        accessory.id
      ];
      final multiplierRebirthAndItem =
          container.read(totalMultiplierProvider).toDouble();
    ...
    test('Acessórios devem aumentar hard cap e multiplicador', () {
      ...
      final multiplierWithAccessories =
          container.read(totalMultiplierProvider).toDouble();

      // Acessórios devem SEMPRE aumentar o multiplicador
      expect(
          multiplierWithAccessories, greaterThan(multiplierWithoutAccessories));
    });
    ...
    test('Multiplicador deve reagir a mudanças em tempo real', () {
      ...
      final multiplierBefore =
          container.read(totalMultiplierProvider).toDouble();

      container.read(equippedAccessoriesProvider.notifier).state = [
        accessory1.id
      ];
      final multiplierAfter1 =
          container.read(totalMultiplierProvider).toDouble();
    ...
    test('Sistema aditivo parcial de acessórios deve funcionar', () {
      ...
      double calculateBonus(List<String> equipped) {
        ...
        for (int i = 0; i < equipped.length; i++) {
          final equippedAccessory = allAccessories.firstWhere(
            (acc) => acc.id == equipped[i],
          );
          final baseValue = equippedAccessory.productionMultiplier - 1.0;
      ...
      final multiplier1 =
          container.read(accessoryMultiplierProvider).toDouble();
    ...
    test(
        'Multiplicador total deve usar calculateEffectiveMultiplier corretamente',
        () {
      container.read(rebirthDataProvider.notifier).state =
          const RebirthData(rebirthCount: 10);
      ...
      final rebirthMultiplier =
          container.read(rebirthMultiplierProvider).toDouble();
      final accessoryMultiplier =
          container.read(accessoryMultiplierProvider).toDouble();
      ...
      final expectedTotal = rebirthMultiplier * accessoryMultiplier;
    ...
    test('Remover acessório deve reduzir multiplicador', () {
      ...
      final multiplierWithBoth =
          container.read(totalMultiplierProvider).toDouble();

      container.read(equippedAccessoriesProvider.notifier).state = [
        accessory1.id
      ];
      final multiplierWithOne =
          container.read(totalMultiplierProvider).toDouble();
