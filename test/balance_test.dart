...
void main() {
  ...
  group('Requisitos de Rebirth', () {
    test('Requisitos devem permitir resets mais cedo', () {
      final rebirthReq1 = RebirthTier.rebirth.getRequirement(1);
      final rebirthReq10 = RebirthTier.rebirth.getRequirement(10);

      expect(rebirthReq1, greaterThanOrEqualTo(1e14));
      expect(rebirthReq10, greaterThan(1e20));

      final ascensionReq1 = RebirthTier.ascension.getRequirement(1);
      expect(ascensionReq1, greaterThanOrEqualTo(1e28));
      expect(ascensionReq1, lessThan(1e30));

      final transReq1 = RebirthTier.transcendence.getRequirement(1);
      expect(transReq1, greaterThanOrEqualTo(1e42));
      expect(transReq1, lessThan(1e44));
    });

    test('Crescimento de requisitos deve ser desafiador sem travar', () {
      final ascensionReq1 = RebirthTier.ascension.getRequirement(1);
      final ascensionReq2 = RebirthTier.ascension.getRequirement(2);

      final growth = ascensionReq2 / ascensionReq1;
      expect(growth, greaterThan(10.0));
      expect(growth, lessThan(25.0));
    });
  });

  group('Custos de Geradores', () {
    test('Taxas de crescimento devem continuar progressivas', () {
    ...
    test('Nada Absoluto não deve criar parede de custo extrema', () {
      final paradoxoTemporal = availableGenerators.firstWhere(
        (g) => g.name == 'Paradoxo Temporal',
      );
      final menteSuprema = availableGenerators.firstWhere(
        (g) => g.name == 'Mente Suprema',
      );
      final nadaAbsoluto = availableGenerators.firstWhere(
        (g) => g.name == 'Nada Absoluto',
      );
      final nexusPrimordial = availableGenerators.firstWhere(
        (g) => g.name == 'Nexus Primordial',
      );

      expect(nadaAbsoluto.baseCost.toScientificString(), '1.000000e40');

      final costJump =
          (nadaAbsoluto.getCost(0) / menteSuprema.getCost(0)).toDouble();
      final productionJump =
          (nadaAbsoluto.getProduction(1) / menteSuprema.getProduction(1))
              .toDouble();
      final preNadaBridge =
          (menteSuprema.getProduction(1) / paradoxoTemporal.getProduction(1))
              .toDouble();
      final postNadaCostJump =
          (nexusPrimordial.getCost(0) / nadaAbsoluto.getCost(0)).toDouble();

      expect(costJump, lessThanOrEqualTo(1e9));
      expect(productionJump, greaterThanOrEqualTo(1e8));
      expect(preNadaBridge, greaterThanOrEqualTo(1e8));
      expect(postNadaCostJump, lessThanOrEqualTo(1e8));
    });
  ...
  group('Produção de Geradores', () {
    test('Produção late-game deve ter penalidade moderada', () {
      final lateGameGenerator = availableGenerators.firstWhere(
  ...
  group('Verificações de Balanceamento Geral', () {
    test('Acessórios devem ser necessários para late game', () {
      ...
      double getFinalMultiplier(
          double base, double accessoryValue, int accessoryCount) {
        final capReduction = (accessoryCount * 0.10).clamp(0.0, 1.0);
        final effectiveHardCap = hardCapBase * (1.0 - capReduction);
    ...
    test('Progressão deve ser controlada sem ficar travada', () {
      final oldRebirthMult = 2.0 * 100;
      final newRebirthMult =
          RebirthTier.rebirth.getEffectiveMultiplierGain(100);

      expect(newRebirthMult, lessThan(oldRebirthMult / 10));
      expect(newRebirthMult, greaterThan(1.8));
    });
